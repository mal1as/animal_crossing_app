...
export class RegisterComponent implements OnInit {
    ...
    userToRegister: User = new User(0, '', '', null);
    ...
    repeatedPasswordMatch(): boolean {
        return this.userToRegister.passwordHash !== this.repeatedPassword;
    }

    hasFormErrors(): boolean {
        return this.registerForm === null || this.registerForm.invalid;
    }

    fieldErrors(field: string): ValidationErrors | null {
        if(this.registerForm === null) return null;
    ...
    onSignUpClick(): void {
        this.registerService.register(this.userToRegister).subscribe(
            ...
            (createdUser: User) => {
                NetworkUtil.authSuccess(createdUser, this.userToRegister);
            },
            error => {
                // if username taken
